...
public class Teleports {
	...
	public static boolean withdrawItems(Teleports.Type type){
		if(canTeleport(type))
			return true;
		if(!type.needAllItems){
			for(int i = type.items.length - 1; i >= 0; i--){
				if(Bank.withdraw(type.items[i], 1)){
					return true;
				}
			}
			return false;
	...
	public static boolean canTeleport(Teleports.Type type){
		if(Skills.getLevel(Skills.MAGIC) < type.magicLevel)
			return false;
		if(!type.needAllItems)
			return Inventory.containsOneOf(type.items) || Equipment.containsOneOf(type.items);
		for(int i = 0, j = countOfX(type.items, i); i < type.items.length; i += j){
	...
	public static boolean teleport(Teleports.Type type){
		return false;
